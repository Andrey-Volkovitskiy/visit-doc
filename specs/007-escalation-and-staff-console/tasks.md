...
exist; and US1's P1 abstention→call-to-staff depends on retrieval being correctly scoped, so the read
...
question — that the abstention hands the question to staff, and that the conversation still
answers the next question it can.
...
- [X] T117 [P] [US5] Write the corpus-effect round trip in `tests/integration/test_faq_session_isolation.py`: after adding an entry, 100% of matching questions are answered with a citation to it; after editing it, the cited text is the **new** text; and after deleting it, the questions it alone supported end in **abstention** — which hands the question to staff rather than producing an unsupported answer (SC-016, US5 scenario 4, FR-003). This is the one task that proves the write path and the read path agree end to end; observe failing. **Since revised**: it asserts the mark and the emphasis rather than an escalation, because a corpus gap no longer silences (FR-003d)
...
- **User Story 5 (Phase 7)**: depends on Foundational's read path, and on US1 for the abstention→call-to-staff half of its independent test. Independent of US2, US3 and US4
...
The corpus gap stops silencing the conversation it calls staff for (spec Session 2026-09-04,
FR-003d). This is the 2026-09-01 correction applied to the trigger it left behind, so it moves one
value out of one set and changes one sentence; the escalation, the mark and the emphasis are
untouched.

- [X] T154 Move `EscalationReason.CORPUS_COULD_NOT_ANSWER` out of `_SILENCING` in `services/chat/src/chat/agent/escalation.py`, leaving `PATIENT_ASKED_FOR_PERSON` as the only reason that writes `escalated_at`. Nothing else in the transition changes: the abstention still records its request, still marks its message permanently, and still emphasizes the conversation. Update `EscalationReason`'s docstring in `services/chat/src/chat/domain/models.py`, which names the silencing pair per FR-003d, FR-009
- [X] T155 Rewrite `_ABSTENTION_MESSAGE` in `services/chat/src/chat/agent/answer_faq.py` to the wording FR-005a fixes, and align `compose_answer`'s system prompt and its not-grounded prompt half so a merged reply says the same three things a single-specialist one does. The closing invitation is load-bearing: a handoff that reads like a goodbye leaves the patient believing in a silence that is not there per FR-003b, FR-005a
- [X] T156 Assert the behaviour the change exists for, in `services/chat/tests/test_turn_api.py`: a turn that abstains is followed, in the same conversation, by a question the corpus **does** answer — which is answered and cited, with the abstaining message still carrying its permanent mark and the conversation still emphasized. Also assert in `services/chat/tests/test_escalation.py` that `corpus_could_not_answer` alone resolves to **no** conversation reason and leaves `may_assistant_reply` true, and that a corpus gap beside a failure claims exactly one emphasis between them per FR-003d, SC-001b
- [X] T157 Update the documents the change makes stale in the same commit (Constitution VI): `spec.md` (FR-003, FR-003b, FR-003c, FR-003d, new FR-005a, FR-009, FR-027c's grid, FR-027d, SC-001a, new SC-001b, SC-009f, SC-015f, SC-016, the Edge Cases and Assumptions that named the pair), `data-model.md`'s two tables, `contracts/agent-tools.md`'s caller table, `contracts/http-api.md`'s `escalation_reason` row and example, `research.md` #1 and #6, `plan.md`, `quickstart.md` scenarios 5 and the prerequisites, and `README.md`'s "Two silences, two columns"
